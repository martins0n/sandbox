...
def pd_series_to_ts(ts: pd.Series):
    ...
    if ts.index.freq == 'M':
        return rstats.ts(
            ...
            end=IntVector((end_year, end_month)), frequency=12
        )
        ...
        return rstats.ts(
            FloatVector(ts.values),
            start=IntVector((start_year, start_month)),
            end=IntVector((end_year, end_month)), frequency=4
        )
        ...
        return rstats.ts(
            ...
            end=IntVector((end_index)), frequency=1
        )
...
def make_prediction(ts: pd.Series, model_path, h=6, full=False):
    ...
    if full:
        ...
        return pd.Series(
            ...
            index=pd.date_range(
                start=ts_end + pd.to_timedelta(ts_freq),
                freq=ts_freq,
                periods=h
            )
        )
