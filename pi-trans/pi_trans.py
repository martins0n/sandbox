...
class PITransformerBlock(nn.Module):
    def __init__(
        self, d_model: int = 512, d_ff: int = 2048, num_heads: int = 4
    ) -> None:
        super().__init__()
...
class LogMeanScaler(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, shift: int, window_size: int, inverse: bool = False
    ) -> torch.Tensor:
        if shift == 0:
...
class PITransformer(nn.Module):
    def __init__(
        ...
        self.transformer = nn.Sequential(
            ...
            *[
                PITransformerBlock(d_model=d_model, d_ff=d_ff, num_heads=num_heads)
    ...
    def forward(
        self, x: torch.Tensor, shift: int, window_size: int, mask=None
    ) -> torch.Tensor:
...
def mask_gen(x, n_heads):
    mask = torch.tril(torch.ones((x.shape[1], x.shape[1])), diagonal=0)
...
if __name__ == "__main__":
    ...
    n_heads = 4
    mask = torch.tril(torch.ones((10, 10)), diagonal=0)
